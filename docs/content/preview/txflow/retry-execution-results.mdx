---
title: "Legacy: Retry, Execution & Results"
description: "Compatibility reference for preview retry, FlowExecutor, and FlowResult APIs"
---

# Legacy: Retry, Execution & Results

> **Compatibility API:** This page documents preview `FlowExecutor` and `FlowResult` types. For new
> code, see [Execution Engine](./execution-engine) and
...
| `withConfirmationConfig(ConfirmationConfig)` | Enable confirmation tracking and configure depth, timeout, and polling interval |
...
Returns a `FlowHandle` immediately for non-blocking monitoring. Supply an
application-managed executor for every async executor instance. TxFlow does not own or shut it
down. On Java 17, use a normal pool; on Java 21+, a virtual-thread executor is also supported.
