...
# QuickTx API (Recommended for most use cases)
...
> **0.8.0 Preview — Unified Tx API**: In 0.8.0-preview1, all `ScriptTx` operations are now available directly on `Tx`.
> `ScriptTx` is deprecated. See the [Unified Tx API](/docs/preview/unified-tx) documentation for details and migration guide.

> **0.8.0 Preview — YAML Transaction Plans**: Define transactions in YAML with variable substitution and execute them
> through QuickTxBuilder. See the [TxPlan](/docs/preview/txplan/overview) documentation.
