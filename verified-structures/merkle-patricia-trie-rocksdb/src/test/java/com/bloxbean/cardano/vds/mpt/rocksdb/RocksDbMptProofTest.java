    ...
    void inclusionProof_survivesReopen() throws Exception {
        ...
        try {
            ...
            try (RocksDbNodeStore store = new RocksDbNodeStore(dir.toString())) {
                com.bloxbean.cardano.vds.mpt.SecureTrie trie = new com.bloxbean.cardano.vds.mpt.SecureTrie(store, HF);
                trie.put(key, value);
            ...
            try (RocksDbNodeStore store = new RocksDbNodeStore(dir.toString())) {
                com.bloxbean.cardano.vds.mpt.SecureTrie trie = new com.bloxbean.cardano.vds.mpt.SecureTrie(store, HF, root);
                byte[] wire = trie.getProofWire(key).orElseThrow();
                assertThat(trie.verifyProofWire(root, key, value, true, wire)).isTrue();
            }
