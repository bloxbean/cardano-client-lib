        ...
        while (remaining > 0) {
                ...
                try (org.rocksdb.WriteBatch wb = new org.rocksdb.WriteBatch();
                    ...
                    if (recordRoots) {
                        ...
                        if (gcRefcount && (options.refcountEvery <= 1 || (commits % options.refcountEvery) == 0)) {
                            ...
                            com.bloxbean.cardano.vds.mpt.rocksdb.gc.strategy.RefcountGcStrategy.incrementAll(
                            ...
                            while (versionWindow.size() > options.keepLatest) {
                                ...
                                if (oldRoot != null) {
                                    com.bloxbean.cardano.vds.mpt.rocksdb.gc.strategy.RefcountGcStrategy.decrementAll(
            ...
            if (gcMarkSweep && options.gcInterval > 0 && (commits % options.gcInterval) == 0 && stateTrees != null) {
                try {
                    ...
                    var gcManager = new com.bloxbean.cardano.vds.mpt.rocksdb.gc.GcManager(
                            (RocksDbNodeStore) stateTrees.nodeStore(), stateTrees.rootsIndex());
                    var policy = com.bloxbean.cardano.vds.mpt.rocksdb.gc.RetentionPolicy.keepLatestN(options.keepLatest);
                    var gcOpts = new com.bloxbean.cardano.vds.mpt.rocksdb.gc.GcOptions();
                    gcOpts.deleteBatchSize = 20_000;
                    gcOpts.useSnapshot = true;
                    var report = gcManager.runSync(new com.bloxbean.cardano.vds.mpt.rocksdb.gc.strategy.OnDiskMarkSweepStrategy(), policy, gcOpts);
