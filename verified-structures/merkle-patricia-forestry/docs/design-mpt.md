# Merkle Patricia Trie - Internal Implementation

**Module:** `merkle-patricia-forestry`

> **Note:** This document describes internal algorithm details. For the recommended API, use `MpfTrie` - see [design.md](design.md).

## Not Ethereum Compatible

This implementation uses **Blake2b-256** and **CBOR encoding**, not Ethereum's Keccak-256 and RLP. While the radix tree structure is similar, proofs are **not interoperable** with Ethereum.
...
```
INSERT(key, value):
1. Convert key to nibbles (MpfTrie hashes keys automatically)
