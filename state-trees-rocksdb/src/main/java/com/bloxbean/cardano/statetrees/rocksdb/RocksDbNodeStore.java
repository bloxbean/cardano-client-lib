...
public class RocksDbNodeStore implements NodeStore, AutoCloseable {
  ...
  public RocksDbNodeStore(String dbPath) {
    try {
      ...
      for (byte[] cfName : existingCfNames) if (Arrays.equals(cfName, RocksDB.DEFAULT_COLUMN_FAMILY)) { hasDefaultCf = true; break; }
      if (!hasDefaultCf) cfDescriptors.add(0, new ColumnFamilyDescriptor(RocksDB.DEFAULT_COLUMN_FAMILY, columnFamilyOptions));
      if (nodesColumnFamilyIndex < 0) { cfDescriptors.add(new ColumnFamilyDescriptor(CF_NODES.getBytes(), columnFamilyOptions)); nodesColumnFamilyIndex = cfDescriptors.size() - 1; }
  ...
  public void put(byte[] hash, byte[] nodeBytes) {
    try {
      ...
      if (currentBatch != null) {
        ...
        java.util.Map<String, byte[]> stagedWrites = TL_STAGED.get();
        if (stagedWrites == null) { stagedWrites = new java.util.HashMap<>(); TL_STAGED.set(stagedWrites); }
   ...
   */
  public ColumnFamilyHandle nodesHandle() { return cfNodes; }
