...
public class RocksDbRootsIndex implements RootsIndex, AutoCloseable {
  ...
  public RocksDbRootsIndex(String dbPath) {
    try {
      ...
      if (rootsColumnFamilyIndex < 0) { cfDescriptors.add(new ColumnFamilyDescriptor(CF_ROOTS.getBytes(), columnFamilyOptions)); rootsColumnFamilyIndex = cfDescriptors.size() - 1; }
  ...
  public java.util.NavigableMap<Long, byte[]> listAll() {
    ...
    try (ReadOptions readOptions = new ReadOptions(); RocksIterator iterator = db.newIterator(cfRoots, readOptions)) {
      for (iterator.seekToFirst(); iterator.isValid(); iterator.next()) {
        ...
        if (java.util.Arrays.equals(key, LATEST_KEY) || java.util.Arrays.equals(key, LAST_VERSION_KEY)) continue;
