...
public final class RocksDbStateTrees implements AutoCloseable {
  ...
  public RocksDbStateTrees(String dbPath) {
    try {
      ...
      for (byte[] cfName : existingCfNames) {
        ...
        if (java.util.Arrays.equals(cfName, RocksDbNodeStore.CF_NODES.getBytes())) nodesColumnFamilyIndex = cfDescriptors.size() - 1;
        if (java.util.Arrays.equals(cfName, RocksDbRootsIndex.CF_ROOTS.getBytes())) rootsColumnFamilyIndex = cfDescriptors.size() - 1;
      }
      // Ensure default CF present
      boolean hasDefaultCf = false;
      for (byte[] cfName : existingCfNames) if (java.util.Arrays.equals(cfName, RocksDB.DEFAULT_COLUMN_FAMILY)) { hasDefaultCf = true; break; }
      if (!hasDefaultCf) cfDescriptors.add(0, new ColumnFamilyDescriptor(RocksDB.DEFAULT_COLUMN_FAMILY, columnFamilyOptions));
      // Ensure nodes/roots CFs present
      if (nodesColumnFamilyIndex < 0) { cfDescriptors.add(new ColumnFamilyDescriptor(RocksDbNodeStore.CF_NODES.getBytes(), columnFamilyOptions)); nodesColumnFamilyIndex = cfDescriptors.size() - 1; }
      if (rootsColumnFamilyIndex < 0) { cfDescriptors.add(new ColumnFamilyDescriptor(RocksDbRootsIndex.CF_ROOTS.getBytes(), columnFamilyOptions)); rootsColumnFamilyIndex = cfDescriptors.size() - 1; }
   ...
   */
  public RocksDbNodeStore nodeStore() { return nodeStore; }
   ...
   */
  public RocksDbRootsIndex rootsIndex() { return rootsIndex; }
  ...
  public void close() {
    ...
    try { cfNodes.close(); } catch (Exception ignored) { /* Ignore cleanup exceptions */ }
    try { cfRoots.close(); } catch (Exception ignored) { /* Ignore cleanup exceptions */ }
    try { db.close(); } catch (Exception ignored) { /* Ignore cleanup exceptions */ }
    try { options.close(); } catch (Exception ignored) { /* Ignore cleanup exceptions */ }
  }
}
