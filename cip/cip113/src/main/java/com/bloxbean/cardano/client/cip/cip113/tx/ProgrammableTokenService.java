...
public interface ProgrammableTokenService {
    ...
    /**
     * A transaction already wired to this deployment — registry lookup, UTxO supplier, script
     * resolver and the protocol's own UTxOs — leaving the caller only the token's concerns.
     */
    ProgrammableTokenTx tx();

    /**
     * This deployment's script resolver, shared by every transaction {@link #tx()} builds.
     *
    ...
    /** The registry lookup backing {@link #getRegistryNode}; exposed so a caller can cache or reset it. */
