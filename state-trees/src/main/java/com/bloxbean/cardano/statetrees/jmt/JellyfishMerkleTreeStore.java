...
public final class JellyfishMerkleTreeStore {
  ...
  public Optional<Long> latestVersion() { return engine.latestVersion(); }
  public byte[] latestRootHash() { return engine.latestRootHash(); }
  public byte[] rootHash(long version) { return engine.rootHash(version); }
  public synchronized JellyfishMerkleTree.CommitResult commit(long version, Map<byte[], byte[]> updates) { return engine.commit(version, updates); }
  public byte[] get(byte[] key) { return engine.get(key); }
  public byte[] get(byte[] key, long version) { return engine.get(key, version); }
  public Optional<JmtProof> getProof(byte[] key, long version) { return engine.getProof(key, version); }
  public Optional<byte[]> getMpfProofCbor(byte[] key, long version) { return engine.getMpfProofCbor(key, version); }
  public PruneReport prune(long versionInclusive) { return engine.prune(versionInclusive); }
  ...
  static final class ReferenceEngine implements Engine {
    ...
    @Override public Optional<Long> latestVersion() { return delegate.latestVersion(); }
    @Override public byte[] latestRootHash() { return delegate.latestRootHash(); }
    @Override public byte[] rootHash(long version) { return delegate.rootHash(version); }
    ...
    private void persist(JellyfishMerkleTree.CommitResult result) {
      try (JmtStore.CommitBatch batch = store.beginCommit(result.version(), JmtStore.CommitConfig.defaults())) {
        ...
        for (JellyfishMerkleTree.CommitResult.ValueOperation op : result.valueOperations()) {
          switch (op.type()) {
            case PUT: batch.putValue(op.keyHash(), op.value()); break;
            case DELETE: batch.deleteValue(op.keyHash()); break;
          }
        }
    ...
    @Override public byte[] get(byte[] key) { return delegate.get(key); }
    @Override public byte[] get(byte[] key, long version) { return delegate.get(key, version); }
    @Override public Optional<JmtProof> getProof(byte[] key, long version) { return delegate.getProof(key, version); }
    @Override public Optional<byte[]> getMpfProofCbor(byte[] key, long version) { return delegate.getMpfProofCbor(key, version); }
  ...
  static final class StreamingEngine implements Engine {
    ...
    public byte[] get(byte[] key, long version) {
      ...
      Optional<byte[]> cached = cachedValue(keyHash);
      if (cached.isPresent()) {
        return cached.get().clone();
      }
      return store.getValueAt(keyHash, version)
          .map(value -> {
            cacheValue(keyHash, value);
            return value.clone();
          })
          .orElse(null);
    }
    ...
    private java.util.Optional<byte[]> cachedValue(byte[] keyHash) {
      if (valueCache == null) {
        return java.util.Optional.empty();
      }
      byte[] cached = valueCache.get(new ByteArrayWrapper(keyHash));
      return cached == null ? java.util.Optional.empty() : java.util.Optional.of(cached);
    }

    private void cacheValue(byte[] keyHash, byte[] value) {
      if (valueCache == null) {
        return;
      }
      valueCache.put(new ByteArrayWrapper(keyHash), value.clone());
    }
    ...
    private void evictValue(byte[] keyHash) {
      ...
      valueCache.remove(new ByteArrayWrapper(keyHash));
    }
    ...
    private final class WorkingSet {
      ...
      private void collide(Traversal traversal,
        ...
        cacheValue(newKeyHash, newValue);
      ...
      private void cacheValue(byte[] keyHash, byte[] value) {
        StreamingEngine.this.cacheValue(keyHash, value);
      }
    ...
    private static final class ValueCache extends java.util.LinkedHashMap<ByteArrayWrapper, byte[]> {
      private final int maxEntries;
      ...
      @Override
      protected boolean removeEldestEntry(java.util.Map.Entry<ByteArrayWrapper, byte[]> eldest) {
        return size() > maxEntries;
      }
    }
    ...
    private static final class NodeAddress {
      ...
      com.bloxbean.cardano.statetrees.common.NibblePath path() { return path; }
