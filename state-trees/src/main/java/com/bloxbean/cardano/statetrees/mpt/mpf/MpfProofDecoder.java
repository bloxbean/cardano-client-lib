...
final class MpfProofDecoder {

    private MpfProofDecoder() {
    }

    static MpfProof decode(byte[] cborBytes) {
        try {
    ...
    private static MpfProof.Step decodeStep(Array array) {
        ...
        if (tag == null || tag.getValue() == 121) {
            ...
            return new MpfProof.BranchStep(skip, neighbors, 0, branchValueHash);
