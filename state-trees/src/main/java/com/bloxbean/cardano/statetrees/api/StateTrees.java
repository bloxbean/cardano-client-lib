...
/**
 * Unified interface for managing Merkle Patricia Trie state across different storage backends.
 *
 * <p>This interface provides a storage-agnostic abstraction for MPT state management,
 * enabling polymorphic usage of different backends (RocksDB, RDBMS, Redis, etc.) with
 * a consistent API.</p>
 *
 ...
 * MerklePatriciaTrie trie = new MerklePatriciaTrie(trees.nodeStore(), Blake2b256::digest);
 *
 * // Perform operations
 * trie.put(key, value);
...
public interface StateTrees extends AutoCloseable {
    ...
    /**
     * Returns the NodeStore for persisting Merkle Patricia Trie nodes.
     *
     * <p>The NodeStore provides content-addressed storage where nodes are
     * identified by their hash. All MPT implementations share the same
     * NodeStore interface regardless of backend.</p>
     *
