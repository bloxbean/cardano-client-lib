...
/**
 * YAML serialization/deserialization tests for unified Tx with collectFrom intentions.
 */
class UnifiedTxYamlRoundTripTest {

    @Test
    void tx_collectFrom_intention_serializes_to_yaml() {
        // Given a Tx with a script collectFrom intention
        ...
        assertThat(entry).containsKey("tx");
        Map<String, Object> content = (Map<String, Object>) entry.get("tx");
        assertThat(content).containsKey("intents");
    }

    @Test
    void tx_collectFrom_without_redeemer_or_datum_serializes_intention() {
        ...
        Tx tx = new Tx()
                .collectFrom(scriptUtxo, (PlutusData) null)
    ...
    @Test
    void tx_collectFrom_yaml_round_trip_restores_intention() {
        // Given
        Utxo scriptUtxo = Utxo.builder()
