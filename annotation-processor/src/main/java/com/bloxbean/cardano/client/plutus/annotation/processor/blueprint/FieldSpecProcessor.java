...
public class FieldSpecProcessor {
    ...
    private String extractBaseType(String key) {
        ...
        if (angleIndex > 0) {
            key = key.substring(0, angleIndex);
        }
    ...
    protected String resolveClassNameFromRef(String ref, BlueprintSchema refSchema) {
        ...
        // If this is a built-in generic container (List, Option, etc.) that was skipped
        // during class generation, return null to signal PlutusData fallback
        if (isBuiltInGenericContainer(simpleName)) {
            return null;  // Signal to use PlutusData
        }

        // For domain-specific types (Interval, IntervalBound, etc.),
        // use the base type to get class name
        // PREFER: Class name from definition key (base type for generics)
        String keyClassName = BlueprintUtil.getClassNameFromReferenceKey(baseType);
        if (keyClassName != null && !keyClassName.isEmpty()) {
            return keyClassName;
        }

        // FALLBACK: Title from referenced schema (for backward compatibility)
        return refSchema != null ? refSchema.getTitle() : null;
    }
     ...
     */
    private boolean isBuiltInGenericContainer(String simpleName) {
    ...
    public ClassName getInnerDatumClass(String ns, BlueprintSchema schema) {
        ...
        // When processing a definition, schema.getRef() is null, so use the namespace parameter
        // When processing a field reference, schema.getRef() is set, extract namespace from it
        ...
        if (schema.getAnyOf() != null && schema.getAnyOf().size() == 1) {
            // When processing a definition (not a field reference), schema.getRef() is null
            // In this case, use the schema's title which was resolved from the definition key
            // When processing a field reference, schema.getRef() is set
            // In this case, use the definition key from the $ref (CIP-57 source of truth)
    ...
    public Tuple<FieldSpec, ClassName> createDatumFieldSpec(String ns, String interfaceName, BlueprintSchema schema, String title) {
        ...
        if (schema.getRefSchema() != null) {
            ...
            // Resolve from definition key instead of ref schema title
            // For domain-specific generic instantiations (e.g., "Interval$Int"), this extracts the base type ("Interval")
            // and uses the typed class that was generated for it
            // For built-in containers (e.g., "Option<T>", "List<T>"), this returns null to signal PlutusData fallback
            String refClassName = resolveClassNameFromRef(schema.getRef(), schema.getRefSchema());

            // If refClassName is null, it means the $ref points to a built-in container
            // that was skipped during class generation OR a truly unresolvable type
            // This is a safety net - most Option<T> types should now be handled by
            // PlutusBlueprintLoader setting dataType=option and routing through OptionDataTypeProcessor
