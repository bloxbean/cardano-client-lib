...
/**
 * Code generation strategy for {@link LocalDate} fields.
 *
 * <ul>
 *   <li>DEFAULT: epoch day as BigInteger</li>
 *   <li>STRING: ISO-8601 text via {@code LocalDate.toString()} / {@code LocalDate.parse()}</li>
 * </ul>
 */
public class LocalDateCodeGen extends AbstractMetadataTypeCodeGen {

    @Override
    public Set<String> supportedJavaTypes() {
        return Set.of(LOCAL_DATE);
    }
    ...
    protected Object[] serializeExpression(String valueExpr, String javaType) {
        return new Object[]{"$T.valueOf(" + valueExpr + ".toEpochDay())", BigInteger.class};
    }

    @Override
    protected Object[] deserializeExpression(String castVar, String javaType) {
        return new Object[]{"$T.ofEpochDay((($T) " + castVar + ").longValue())",
