...
class FieldSpecProcessorTest {
    ...
    class ExtractBaseTypeTests {
        ...
        void shouldStripAngleBracketSyntax() {
            String result = fieldSpecProcessor.extractBaseType("Option<T>");
            assertThat(result).isEqualTo("Option");
        }

        @Test
        void shouldReturnUnchanged_whenNoGenerics() {
            String result = fieldSpecProcessor.extractBaseType("ValidityRange");
            assertThat(result).isEqualTo("ValidityRange");
        }

        @Test
        void shouldReturnNull_whenInputIsNull() {
            String result = fieldSpecProcessor.extractBaseType(null);
            assertThat(result).isNull();
        }

        @Test
        void shouldReturnEmpty_whenInputIsEmpty() {
            String result = fieldSpecProcessor.extractBaseType("");
    ...
    /**
     * Tests for buildInterfaceTypeSpecBuilder() method.
     *
     * <p>Builds a TypeSpec.Builder for a Datum interface with @Constr annotation.</p>
     */
    @Nested
    class BuildInterfaceTypeSpecBuilderTests {

        @Test
        void shouldCreateInterfaceWithCorrectName() {
            TypeSpec.Builder builder = fieldSpecProcessor.buildInterfaceTypeSpecBuilder("Credential");
            TypeSpec typeSpec = builder.build();

            assertThat(typeSpec.name).isEqualTo("Credential");
        }

        @Test
        void shouldHavePublicModifier() {
            TypeSpec.Builder builder = fieldSpecProcessor.buildInterfaceTypeSpecBuilder("Credential");
            TypeSpec typeSpec = builder.build();
        ...
        @Test
        void shouldHaveConstrAnnotation() {
            TypeSpec.Builder builder = fieldSpecProcessor.buildInterfaceTypeSpecBuilder("Credential");
            TypeSpec typeSpec = builder.build();

            assertThat(typeSpec.annotations)
                    .extracting(a -> a.type.toString())
                    .contains(Constr.class.getName());
        }
        ...
        void shouldBeAnInterface() {
            ...
            assertThat(typeSpec.kind).isEqualTo(TypeSpec.Kind.INTERFACE);
        }

        @Test
        void shouldApplyNamingStrategy() {
            // Naming strategy converts to class name format
            TypeSpec.Builder builder = fieldSpecProcessor.buildInterfaceTypeSpecBuilder("my_type");
            TypeSpec typeSpec = builder.build();

            assertThat(typeSpec.name).isEqualTo("MyType");
        }
    }
     ...
     *
     * <p>Builds a TypeSpec for a variant class nested inside an interface.
    ...
    class BuildVariantTypeSpecTests {
        ...
        @Test
        void shouldReturnNull_whenSchemaHasNoDataType() {
            BlueprintSchema schema = new BlueprintSchema();
            // No dataType set
        ...
        @Test
        void shouldCreateStaticAbstractClass() {
            BlueprintSchema schema = new BlueprintSchema();
            ...
            assertThat(result.modifiers).contains(Modifier.PUBLIC, Modifier.STATIC, Modifier.ABSTRACT);
            assertThat(result.kind).isEqualTo(TypeSpec.Kind.CLASS);
        }

        @Test
        void shouldImplementDataAndInterface() {
            BlueprintSchema schema = new BlueprintSchema();
            schema.setDataType(BlueprintDatatype.constructor);
            schema.setTitle("VerificationKey");
            schema.setIndex(0);
            ...
            assertThat(result).isNotNull();
            // Should have 2 superinterfaces: Data<Credential.VerificationKey> and Credential
        ...
        @Test
        void shouldHaveConstrAnnotationWithAlternative() {
            BlueprintSchema schema = new BlueprintSchema();
            schema.setDataType(BlueprintDatatype.constructor);
            schema.setTitle("Script");
            schema.setIndex(1);

            TypeSpec result = fieldSpecProcessor.buildVariantTypeSpec("ns", "Credential", schema);

            assertThat(result).isNotNull();
            AnnotationSpec constrAnnotation = result.annotations.stream()
                    .filter(a -> a.type.toString().contains("Constr"))
                    .findFirst()
                    .orElse(null);
            assertThat(constrAnnotation).isNotNull();
            assertThat(constrAnnotation.members.toString()).contains("1");
        }
        ...
        @Test
        void shouldUseCorrectVariantName() {
            BlueprintSchema schema = new BlueprintSchema();
            schema.setDataType(BlueprintDatatype.constructor);
            schema.setTitle("VerificationKey");
            schema.setIndex(0);
