package com.bloxbean.cardano.client.plutus.annotation.blueprint.model;

import com.bloxbean.cardano.client.plutus.annotation.Blueprint;
import com.bloxbean.cardano.client.plutus.aiken.annotation.AikenStdlib;
import com.bloxbean.cardano.client.plutus.aiken.annotation.AikenStdlibVersion;

@Blueprint(fileInResources = "blueprint/recurring_payment_aiken_v1_1_3_3d77b5c.json", packageName = "com.bloxbean.cardano.client.plutus.annotation.blueprint.model.recurringpayment")
@AikenStdlib(AikenStdlibVersion.V3)
public interface RecurringPaymentBlueprint {
}
